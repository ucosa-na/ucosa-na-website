...
const { normalizePhone, sendSMS } = require('../sms');
  ...
  return `
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;color:#333;">
      <div style="background:#1a1a2e;padding:28px 32px;border-radius:10px 10px 0 0;text-align:center;">
        <img src="https://ucosa-na.org/logo.jpg" alt="UCOSA-NA Logo" style="width:80px;height:80px;border-radius:50%;border:3px solid #c8a96e;display:block;margin:0 auto 10px">
        <div style="color:#c8a96e;font-size:0.85em;letter-spacing:2px;text-transform:uppercase">UCOSA North America</div>
        <h1 style="color:#fff;margin:8px 0 0;font-size:20px;">Annual Dues Reminder</h1>
      </div>
      <div style="background:#f9f9f9;padding:28px 32px;border-radius:0 0 10px 10px;">
...
// POST /api/admin/meeting/invite-test — send meeting invite to a single recipient for testing
router.post('/meeting/invite-test', secOrAdmin, async (req, res) => {
  ...
  try {
    if (email) {
      await sendEmail({
        to: email,
        subject: `[TEST] UCOSA-NA Monthly General Meeting — ${formatted}`,
        html: `
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/>
<style>
  body{font-family:'Segoe UI',Arial,sans-serif;background:#f5f0eb;margin:0;padding:0}
...
<div class="wrap">
  ...
  <div class="ftr">&copy; ${new Date().getFullYear()} UCOSA-North America. All rights reserved. &mdash; <a href="https://ucosa-na.org">ucosa-na.org</a></div>
    ...
    if (phone) {
      ...
      await sendSMS(e164, `[TEST] Dear ${memberName}, you are invited to the UCOSA-NA Monthly General Zoom Meeting on ${formatted} at 5:00 PM Eastern. Join: https://us02web.zoom.us/j/88274188382?pwd=RzVEUFZRYWYxUVl4dkFZdEVBdzhlQT09 | ID: 882 7418 8382 | Passcode: 161773 — UCOSA-NA`);
...
// GET /api/admin/join-requests — list all membership interest submissions
...
router.get('/special-levies', requireAuth, async (req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT sl.id, sl.type, sl.year, sl.amount, sl.amount_due, sl.paid_date, sl.donation_code, sl.notes,
             sl.user_id, u.full_name AS member_name,
...
router.post('/special-levies', finOrAdmin, async (req, res) => {
  const { userId, type, year, amount, amountDue, paidDate, donationCode, notes } = req.body;
  ...
  try {
    const levyAmountDue = type === 'Development Levy' ? parseFloat(amountDue || 200) : null;
    const { rows } = await pool.query(`
      INSERT INTO special_levies (user_id, type, year, amount, amount_due, paid_date, donation_code, notes, recorded_by)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
      RETURNING id
...
router.put('/special-levies/:id', finOrAdmin, async (req, res) => {
  const { type, year, amount, amountDue, paidDate, donationCode, notes } = req.body;
  ...
  try {
    ...
    const { rowCount } = await pool.query(`
      ...
      SET type=$1, year=$2, amount=$3, amount_due=$4, paid_date=$5, donation_code=$6, notes=$7, updated_at=NOW()
      WHERE id=$8
    `, [type, year, parseFloat(amount), levyAmountDue, paidDate || null, donationCode || null, notes || null, req.params.id]);
    if (!rowCount) return res.status(404).json({ error: 'Record not found.' });
    if (before[0]) {
...
router.get('/birthdays', adminOnly, async (req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT mb.id, mb.user_id, mb.member_name, mb.birthday_month, mb.photo_url
...
// PUT /api/admin/birthdays/:id — update a member's birthday month and photo
router.put('/birthdays/:id', adminOnly, async (req, res) => {
  ...
  const { birthday_month, photo_url } = req.body;
  if (!VALID_MONTHS.includes(birthday_month)) return res.status(400).json({ error: 'Invalid month' });
  try {
    const { rows } = await pool.query(
      `UPDATE members_birthday SET birthday_month = $1, photo_url = $2 WHERE id = $3 RETURNING *`,
      [birthday_month, photo_url || null, req.params.id]
    );
    if (!rows.length) return res.status(404).json({ error: 'Record not found' });
    await logAudit(req.user.id, req.user.email, 'BIRTHDAY_UPDATED', 'MEMBER', rows[0].user_id, rows[0].member_name, { birthday_month, photo_url });
    res.json(rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// POST /api/admin/users/send-activation-reminders — reset passwords + email/SMS unactivated members
router.post('/users/send-activation-reminders', secOrAdmin, async (req, res) => {
  try {
    const { rows } = await pool.query(`
    ...
    `);
    if (!rows.length) return res.json({ sent: 0, message: 'No unactivated members found.' });
    ...
    for (const user of rows) {
        ...
        html: `
          <div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;border-radius:12px;overflow:hidden;border:1px solid #e8d9c0">
            ...
            <div style="background:#fdf6ec;padding:32px">
              ...
              <p style="color:#7b2152"><strong>You will be asked to set a new password after logging in.</strong></p>
              <p style="color:#888;font-size:0.85em;margin-top:24px">
                Questions? Contact us at <a href="mailto:ucosa.northamerica@gmail.com">ucosa.northamerica@gmail.com</a>.
              </p>
            </div>
          </div>
        `,
      }).catch(err => log.error(`Activation reminder email failed for ${user.email}: ${err.message}`));
      ...
      if (user.phone) {
        sendSMS(user.phone,
          ...
          `Email: ${user.email}\n` +
          `Temp password: ${tempPassword}\n` +
          `Expires in 48 hours. Please log in and change it.`
        ).catch(err => log.error(`Activation reminder SMS failed for ${user.phone}: ${err.message}`));
      }

      await logAudit(req.user.id, req.user.email, 'ACTIVATION_REMINDER_SENT', 'MEMBER', user.id, user.full_name, { email: user.email });
      results.push({ id: user.id, name: user.full_name, email: user.email });
    }

    res.json({ sent: results.length, members: results });
  } catch (err) {
    log.error('Activation reminders error:', err.message);
    res.status(500).json({ error: 'Internal server error' });
  }
});

// POST /api/admin/send-enrollment-email — manually trigger endowment enrollment open email
router.post('/send-enrollment-email', adminOnly, async (req, res) => {
  try {
    ...
    if (!members.length) return res.json({ sent: 0, failed: 0 });
...
<div class="wrap">
  ...
  <div class="ftr">&copy; ${year} UCOSA-North America. All rights reserved. &mdash; <a href="https://ucosa-na.org">ucosa-na.org</a></div>
    ...
    await logAudit(req.user.id, req.user.email, 'ENROLLMENT_EMAIL_SENT', 'SYSTEM', null, null, { sent, failed, year });
    log.info(`Manual enrollment email: ${sent} sent, ${failed} failed`);
...
router.put('/computer-teacher-salary/:id', finOrAdmin, async (req, res) => {
  ...
  try {
    // Fetch previous record to detect status change
    const { rows: prev } = await pool.query('SELECT status FROM computer_teacher_salary WHERE id=$1', [req.params.id]);
    if (!prev.length) return res.status(404).json({ error: 'Record not found.' });
